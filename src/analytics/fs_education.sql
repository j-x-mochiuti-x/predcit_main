WITH tb_usuario_cursos AS (
    SELECT idUsuario,
            descSlugCurso,
            COUNT(descSlugCursoEpisodio) AS qtdeEpisodios
...
tb_cursos_total_eps AS (
    SELECT descSlugCurso,
            COUNT(descEpisodio) AS qtTotalEps
...
SELECT t1.idUsuario,
        t1.descSlugCurso,
        1. * t1.qtdeEpisodios / t2.qtTotalEps AS pctCursoCompleto
...
SUM(CASE WHEN pctCursoCompleto = 1 THEN 1 ELSE 0 END) AS qtCursosCompletos,
SUM(CASE WHEN pctCursoCompleto > 0 AND pctCursoCompleto < 1 THEN 1 ELSE 0 END) AS qtCursosIncompletos,
...
tb_join AS (
    SELECT t3.idTMWCliente AS idCliente,
        t1.qtCursosCompletos,
        t1.qtCursosIncompletos,
        t1.carreira,
        t1.coletaDados2024,
        t1.dsDatabricks2024,
        t1.dsPontos2024,
